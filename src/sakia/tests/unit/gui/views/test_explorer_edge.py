...
class TestExplorerEdge(unittest.TestCase, QuamashTest):
    ...
    def test_create_wot_edge(self):
        ...
        async def exec_test():
            ...
            self.assertAlmostEqual(edge.destination_point.x(), 20.0)
            self.assertAlmostEqual(edge.destination_point.y(), 40.0)
            self.assertAlmostEqual(edge.source_point.x(), 0.0)
            self.assertAlmostEqual(edge.source_point.y(), 10.0)
    ...
    def test_bounding_rect(self, painter, widget):
        ...
        async def exec_test():
            ...
            self.assertAlmostEqual(bounding_rect.y(), 7.0)
            self.assertAlmostEqual(bounding_rect.width(), 26.0)
            self.assertAlmostEqual(bounding_rect.height(), 36.0)
